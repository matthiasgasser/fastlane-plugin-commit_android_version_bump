    ...
    class CommitAndroidVersionBumpAction < Action
      def self.run(params)
        ...
        if build_file_folder != nil
            ...
            build_file_path = build_file_folder+"/build.gradle.kts"
            ...
            if build_folder_paths.count > 1
                UI.user_error!("Found multiple build.gradle.kts projects in the current repository's working directory.")
            end
            ...
            build_file_paths = Dir[File.expand_path(File.join('**/', app_folder_name, 'build.gradle.kts'))].reject { |file| file.include? 'node_modules/' }

            # no build.gradle found: error
            UI.user_error!('Could not find a build.gradle.kts in the current repository\'s working directory.') if
      ...
      def self.description
        "This Android plugins allow you to commit every modification done in your build.gradle.kts file during the execution of a lane. In fast, it do the same as the commit_version_bump action, but for Android"
      end
